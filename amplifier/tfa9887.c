        ...
        uint16_t *val)
{
    ...
    if ((rc = ioctl(amp->fd, TFA9887_WRITE_CONFIG, &reg_val)) != 0) {
        ALOGE("%s: ioctl %d failed, rc = %d",
                __func__, TFA9887_WRITE_CONFIG, -errno);
        goto read_reg_err;
    }
    ...
    if ((rc = ioctl(amp->fd, TFA9887_READ_CONFIG, &reg_val)) != 0) {
        ALOGE("%s: ioctl %d failed, rc = %d",
                __func__, TFA9887_READ_CONFIG, -errno);
        goto read_reg_err;
    }
        ...
        int len)
{
    ...
    reg_buf[1] = (0xFF & addr);
    if ((rc = ioctl(amp->fd, TFA9887_WRITE_CONFIG, &reg_val)) != 0) {
        ALOGE("ioctl %d failed, ret = %d", TFA9887_WRITE_CONFIG, rc);
        goto read_err;
    }
        ...
        int num_words, uint32_t *p_values)
{
    ...
    if (rc != 0) {
        ALOGE("%s: error reading from register CF_CONTROLS: %d\n",
                __func__, rc);
        goto read_mem_err;
    }
    ...
    if (rc != 0) {
        ALOGE("%s: error writing to register CF_CONTROLS: %d\n",
                __func__, rc);
        goto read_mem_err;
    }
    rc = tfa9887_write_reg(amp, TFA9887_CF_MAD, start_offset);
    if (rc != 0) {
        ALOGE("%s: error writing to register CF_MAD: %d\n",
                __func__, rc);
        goto read_mem_err;
    }
    ...
    for (; num_bytes > 0; ) {
        ...
        if (rc != 0) {
            ALOGE("%s: error reading from CF_MEM: %d\n",
                    __func__, rc);
            goto read_mem_err;
        }
...
static int tfa9887_select_input(struct tfa9887_amp_t *amp, int input)
{
    ...
    switch (input) {
        ...
        case 2:
            ...
            break;
        default:
            ALOGW("%s: Invalid input selected: %d",
                    __func__, input);
            rc = -1;
            goto select_amp_err;
    }
...
static int tfa9887_select_channel(struct tfa9887_amp_t *amp, int channels)
{
    ...
    switch (channels) {
        ...
        case 2:
            ...
            break;
        default:
            ALOGW("%s: Too many channels requested: %d",
                    __func__, channels);
            rc = -1;
            goto select_channel_err;
    }
    ...
    if (rc == 0) {
        ...
        switch (sample_rate) {
            ...
            case 8000:
                ...
                break;
            default:
                ALOGE("%s: Unsupported sample rate %d",
        ...
        uint32_t ready_bits, uint32_t ready_state)
{
    ...
    do {
        ...
        ALOGV("%s: Waiting for 0x%04x, current state: 0x%04x",
                __func__, ready_state, value);
        tries++;
        usleep(1000);
    } while (!ready && tries < 10);

    if (tries >= 10) {
        ALOGE("%s: Timed out waiting for tfa9887 to become ready",
                __func__);
        rc = -1;
    }
...
int tfa9887_set_mode(audio_mode_t mode)
{
    ...
    for (i = 0; i < AMP_MAX; i++) {
        ...
        if (dsp_mode == amp->mode) {
            ALOGV("%s: No mode change needed, already mode %d",
                    __func__, dsp_mode);
            continue;
        }
